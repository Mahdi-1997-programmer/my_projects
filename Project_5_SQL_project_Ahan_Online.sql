----------------------------------
--Ahan_online entrance exam (2022)|
----------------------------------
...
select cast((@total_sale_no_profit/ @total_sale_with_profit) * 100 as decimal(4,2)) as Profit_percentage
------------------------------------------------------------------------------------
...
with r_cte as(		--In this "Personnel chart", 'levels_' and 'head_manager_id' for each employee are displayed.
	select *, cast([name] as varchar(max)) as managers, id as boss_id from personnel where [manager_id] is null  --***or 'where [id] = 1', to display only from specific node.
	UNION ALL
	select p.*, cast(t.[managers]+ ',' + p.[name] as varchar(max)) as managers, t.boss_id as boss_id from personnel as p 
	inner join r_cte as t
		on p.manager_id = t.id)
select id, [name], manager_id, case
	when id in (1,2) then null
	else managers
	end as managers, case
